...
def CTF(x):
    if len(x) < 2:
        raise ValueError("CTF function requires at least 2 dimensions but received: {}".format(len(x)))

    # Access the first two dimensions correctly
    x1 = x.iloc[0]  # First dimension
    x2 = x.iloc[1]  # Second dimension

    # Compute function components
    a1 = x1 ** 2
    a2 = x2 ** 2
    a3 = a1 + a2
    a = np.abs(100 - np.sqrt(a3 / np.pi))

    b1 = np.sin(x1 * (math.pi / 180)) * np.sin(x2 * (math.pi / 180))
    b = np.abs(b1 * np.exp(a)) + 1

    # Compute final result
    c = -0.0001 * (b ** 0.1)

    return float(c)   # Ensure the output is a single scalar value
...
# Example usage:
xx = [0,-1]  # Random 6-dimensional vector
result = goldstein_price(xx)
